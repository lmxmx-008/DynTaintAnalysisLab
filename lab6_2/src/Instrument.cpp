...
namespace instrument{
	...
	void instrumentTrunc(Instruction &I,std::string destStr,std::string srcStr) {
    	if (auto *truncInst = dyn_cast<TruncInst>(&I)) {
			...
			*/
			...
			IRBuilder<> builder(truncInst);
			builder.SetInsertPoint(truncInst->getNextNode());
			
			Value *destString = builder.CreateGlobalStringPtr(destStr);
			Value *srcString = builder.CreateGlobalStringPtr(srcStr);

			builder.CreateCall(GEPInstProcess, {destString,srcString});
			}
	}
    ...
    void instrumentLoad(Instruction &I,std::string destStr,std::string srcStr) {
    	if (auto *loadIns = dyn_cast<LoadInst>(&I)) {
			...
			*/
			Value *src = loadIns->getPointerOperand(); 

			Module *M = loadIns->getModule();
			LLVMContext &context = M->getContext();

			auto loadInstProcess = M->getOrInsertFunction(
				"LoadInstProcess",
				Type::getVoidTy(context),
				Type::getInt8Ty(context)->getPointerTo(),
				Type::getInt8Ty(context)->getPointerTo(),
			...
			IRBuilder<> builder(loadIns);
			builder.SetInsertPoint(loadIns->getNextNode());
			...
			builder.CreateCall(loadInstProcess, { castedSrc,destString, srcString});
			}
	}
