	...
	for (inst_iterator Iter = inst_begin(F), E = inst_end(F); Iter != E; ++Iter) {
		...
		*/
		if (isa<BinaryOperator>(&Inst)) {
			instrumentBinaryOperator(Inst,ResultString,FirstOperandString,SecondOperandString);
		}
		else if (isa<LoadInst>(&Inst)) {
			instrumentLoad(Inst,ResultString,FirstOperandString);
		}
		else if (isa<StoreInst>(&Inst)) {
			instrumentStore(Inst,SecondOperandString,FirstOperandString); //note: op2 is dest
		}
		else if (isa<GetElementPtrInst>(&Inst)) {
			instrumentGEP(Inst,ResultString,FirstOperandString); 
		}
		else if (isa<TruncInst>(&Inst)){
			instrumentTrunc(Inst,ResultString,FirstOperandString);
		}
	}
	return true;
}
...
void setTaintSource(Instruction &I){
	...
	if (calledFunc && calledFunc->getName().contains("scanf")) {
		Value *arg = call->getArgOperand(1);
		instrumentTaintSourcePtrVal(I, arg,variable(arg));
	}

	if (calledFunc && (calledFunc->getName() == "getchar")) {
		instrumentTaintSourceVal(I, variable(call));
	}
}
...
void setTaintPropagation(Instruction &I){
	...
	if (calledFunc && calledFunc->getName() == "strcat") {
